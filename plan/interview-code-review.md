...
Интервью состоит из двух упражнений. Первое — симуляция discovery с продакт-оунером. Второе — ревью готового кода, который был написан под похожую задачу.

---

# Упражнение 1 — Discovery с продакт-оунером (~5 минут)

Прежде чем мы посмотрим код — давай смоделируем ситуацию.

**Я — продакт-оунер.** Только что принёс тебе задачу в мессенджер. Пишу:

> «Привет! Нам нужна админка — админ должен выставлять комиссии мерчантам по операциям.
>
> Брокер-сервис уже умеет это принимать. Вот контракт главной ручки:
>
> ```
> POST {broker}/api/v1/fees/create-or-update-fees?broker_id=<int>
> {
>   "fees": [
>     {
>       "broker_user_id": <int>,
>       "value": "0.1",                      // строка, десятичная дробь
>       "operation_type": "WITHDRAWAL_WIRE",
>       "fee_collector_account": {
>         "account_type": "ACCOUNT_TYPE_PLATFORM" | "ACCOUNT_TYPE_BROKER" | "ACCOUNT_TYPE_CUSTOM",
>         "account_name": "platform" | "broker" | "<имя партнёра>",
>         "bank_account_id": "<строка>"
>       }
>     }
>     // ... массив — по одному Fee на комбинацию operation_type × account_type
>   ]
> }
> ```
>
> Ещё у брокера есть три вспомогательные ручки (я в них не силён, глянь сам):
>
> - `GET list/aliases` — возвращает алиасы: имя + набор `operation_type`. Это чтобы админ не тыкал 30 типов вручную, а выбирал «Withdrawal Standard» — и под ним летит пачка операций.
> - `GET list/swap-overrides` — текущие комиссии и дефолты по пользователю. Из неё можно вытащить `bank_account_id` для platform/broker.
> - `GET list/revshare-accounts` — список кастомных аккаунтов партнёров, с именами и `bank_account_id`.
>
> Задача: сделай нам CRM-эндпоинт так, чтобы админ в интерфейсе выбрал мерчанта, алиас (например «Withdrawal Standard»), поставил три цифры — broker fee, platform fee, revshare fee (плюс имя revshare-аккаунта для последней) — и нажал «сохранить». Оно должно приземлиться в брокер-сервис через POST выше, плюс записать в историю кто что поменял.
>
> Когда сможешь?»

Всё — больше я тебе ничего не написал.

**Твоя задача** — собрать достаточно контекста, прежде чем садиться писать код. Задавай мне любые вопросы — я буду отвечать как реальный PO: что-то знаю сразу, что-то «хороший вопрос, надо уточнить», что-то «а как ты сам предлагаешь?».

Конкретных требований ни к формату, ни к количеству вопросов нет. Хочется понять, как ты думаешь перед кодом и что для тебя важно выяснить.

Подумай о:

- **контракте** эндпоинта — что в запросе, что в ответе, какие ошибки, как они выглядят для фронта;
- **требованиях** — функциональных (что должно работать, какие комбинации допустимы, какие инварианты) и нефункциональных (SLA, нагрузка, консистентность, идемпотентность, аудит, права доступа);
- **бизнес-контексте** — кто пользователь, что критично в этой задаче, какие риски у нас если что-то пойдёт не так.

Поехали — задавай свой первый вопрос.

---

# Упражнение 2 — Code Review (~15 минут)
