...
> «Привет! Нам нужна админка — админ должен иметь возможность выставлять комиссии мерчантам. Три типа: `broker`, `platform` и `revshare`. На конкретного мерчанта и конкретный тип операции. Нужен эндпоинт. Когда сможешь?»
