...
> «Привет! Нам нужна админка — админ должен иметь возможность выставлять комиссии мерчантам. Три типа: `broker`, `platform` и `revshare`. На конкретного мерчанта и конкретный тип операции.
>
> Накидал черновик контракта:
>
> ```
> POST /api/v1/fees/create-or-update-fees
> {
>   "aliasName": "Withdrawal Standard",    // тип операции
>   "merchantId": "<uuid мерчанта>",
>   "brokerUserId": 100001,                 // id брокера в платформе
>   "brokerFee": "0.1",                     // опционально
>   "platformFee": "0.2",                   // опционально
>   "revshareFee": "0.05",                  // опционально
>   "revshareAccountName": "partner"        // опционально, для revshare
> }
> 204 при успехе.
> ```
>
> Как это работает: админ открывает мерчанта, видит текущие комиссии по типам операций, правит цифры, сохраняет. Сами комиссии физически лежат во внешнем брокер-сервисе — мы туда прокидываем через их API. Плюс надо записать изменения в историю — кто что поменял.
>
> Когда сможешь?»
